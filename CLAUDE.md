...
- **模型验证**: `sanitizeModel(provider, model)` 检查模型前缀（`claude-`/`gpt-`/`gemini-`），不匹配则静默回退到默认模型；minimax / sub2api / sub2api2 / sub2api3 跳过严格校验（sub2api 系列同时支持 `claude-*` / `gemini-*` / `gpt-*`）
...
- **`gestureKeepMenu` 设置**（`chrome.storage.sync`，boolean，默认 `false`）：
  - **`false`（默认，触控板友好）**：所有平台右键直接进手势模式，contextmenu 始终抑制（`suppressContext = true` 在 mousedown 时设置）。Mac 用户配"左下角=右键"后，按住左下角 + 触控板滑动即触发手势；wheel 事件 handler 把滑动按 `-deltaX/-deltaY` 计入方向序列（自然滚动取反）
  - **`true`（保留菜单）**：
...
- `prompt`, `promptHtml`, `promptCards`, `promptMindmap`, `promptVocab` — 自定义 prompt（注意 summary 的 key 是 `prompt` 而非 `promptSummary`，向后兼容；设置页 UI 已移除编辑入口，但 storage 已写入的值仍生效）
...
- `generateAllCards`, `generateAllVocab` — 「全部生成」是否包含卡片/词汇（boolean，默认 false）
- `enableGestures` — 鼠标手势总开关（boolean，默认 true，`!== false` 判断）
- `gestureKeepMenu` — 是否保留原生右键菜单（boolean，默认 false：右键直接走手势，触控板友好；true 时 Mac 需 Shift+右键，Win/Linux 用 mouseup 时位移判断）
